...
The Swifty Sentry **report-ingest server** — a Koa 3 service that receives data
from the [`@swifty.js/sentry`](../sentry) SDK, persists it as JSONL logs, and
...
- **Immutable-rotated caching** — rotated files are parsed once and cached in an
  `@swifty.js/cache` group (LRU + single-flight), keyed by file size, mtime, and a
